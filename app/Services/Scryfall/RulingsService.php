...
class RulingsService extends ScryfallService
{
    ...
    /**
     * Stream the gzipped JSONL bulk directly from Scryfall and buffer-insert
     * each ruling — no on-disk file, no full-response materialization in
     * PHP memory.
     */
    private function traverseJson(string $downloadUri): void
    {
        ...
        $count = $this->streamJsonl($downloadUri, function (array $ruling): void {
            $this->bufferRuling($ruling);
        });
        $this->flushBuffer();
