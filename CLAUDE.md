...
- Bulk JSON files (~700 MB total — `default_cards` 537 MB + `oracle_cards` 170 MB + `rulings` 25 MB) are only downloaded if they don't already exist on disk. Production deletes them after processing (forcing a fresh download each run); non-production keeps them so subsequent runs skip the download.
...
- Image handling is split into three services with strict separation of concerns:
  ...
  - **Download** (`ImageDownloadService`) — downloads images to disk, no DB writes
