...
- **Deck state outranks mode for per-card badges.** A deck in state `planned` ships `collection_availability` per row instead of the mode-B/C badge, whatever mode it is in — a planned deck asks "can my collection cover this?", not "which stack backs this?". `built` and `archived` decks ship the mode badges as before. Exactly one of `collection_status` / `collection_implicit_status` / `collection_availability` is ever non-null on a row. Availability is owner-only and still obeys the master switch. The deck-header collection-mode badge (and its picker popover) is hidden on planned decks too — the stored mode is untouched and the picker returns once the deck is finished. The deck-state badge carries the legend for whichever set is rendering (`DeckState.vue`'s `explain` prop).
- Availability counts a stack as free unless another deck holds it: a `deck_card_card_stack` pivot row to another deck (explicit), or the stack sitting in a container that is another deck's `decks.container_id` (implicit — any deck, whatever its mode). Green needs `exact >= quantity` of the row's own printing; orange means something is free but not that; red means nothing of the oracle card is free.

**Service-of-record map:**
- `DeckCollectionStatusService` — read-side: resolves `effectiveMode`, ships `statusForDeck` (mode C), `implicitStatusForDeck` (mode B) or `availabilityForDeck` (planned decks, any mode) to the controller. `freeCopiesForOracles($deck, $oracleIds)` exposes the same free/held partition `availabilityForDeck` uses (`oracle id → printing id → free copies`, **newest printing first**) for callers outside the deck page; it honours the master switch itself, where `availabilityForDeck` relies on its controller's gate.
- **Add Cards → "only show printings available in your collection"** (a switch in `CardAddModal`, hidden while the master switch is off, default off): `only_available=1` on the printings endpoint applies `DeckCollectionStatusService::constrainToFreePrintings` — the **SQL twin** of `partitionCopies`' PHP rule, applied before the LIMIT so a narrowed search still fills a page. The rule therefore exists twice; `FreePrintingsConstraintTest::the_sql_filter_and_the_php_rule_agree` pins both to one stated answer on one fixture, so change one and that test fails.
- **Quick Add printing choice:** `DeckCardSearchService::searchOracleCardsForDeck` prefers the printing the deck has most copies of free in the collection (ties go to the newest, which the newest-first map order gives for free), and falls back to the newest printing — the pre-existing rule — when nothing is free or the master switch is off. `preferredPrintingId` is public because the search path orders by `CHAR_LENGTH` and is therefore MariaDB-only, so the preference rule is tested as a pure function instead.
...
- **A spec that never touches the DOM declares `// @vitest-environment node` on its first line.** Building a jsdom per file is the single most expensive thing the suite does — about 0.47s each, well over half of total run time — and the 17 pure-logic specs under `utils/__tests__/` and the non-mounting ones under `composables/__tests__/` were paying it for a `document` they never read. Skipping it takes those files from 1.25s to 0.4s and the whole suite from 5.3s to 4.5s, with per-file isolation completely untouched. `setup.ts` is shared by both environments and guards its prototype patches behind `hasDom`, so a node-env spec loads it without tripping over `Element is not defined`. Add the docblock to a new pure-function spec; **delete it** the moment the spec mounts a component or reads `document` — the failure is a loud `document is not defined`, not a silent pass. It is deliberately a per-file docblock rather than a glob in `vitest.config.ts`: the environment is then visible in the file that depends on it, and a new spec dropped into `utils/__tests__/` gets a DOM by default instead of silently inheriting the wrong environment.
