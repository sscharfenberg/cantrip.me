...
- **Streaming-only.** `TranslationsService` streams the bulk JSON directly from Scryfall via `JsonParser`'s `Endpoint` source (see "Download / parse strategy" below). No on-disk file. Historical note: at design time, this diverged from sibling services (`OracleCardsService`/`DefaultCardsService`/`RulingsService`) which downloaded to a `scryfall-bulk` disk first; those services have since been migrated to the same streaming pattern, and the `scryfall-bulk` disk has been removed.
...
`TranslationsService` parses the bulk via `JsonParser::parse($downloadUri)` — `cerbero/json-parser` exposes `Endpoint` and `Filename` source types under `vendor/cerbero/json-parser/src/Sources/`, and `Endpoint` streams the HTTP response chunk-by-chunk through the parser without ever materializing the full file. No file is written to disk.

**Historical note:** in the original design, the sibling services (`OracleCardsService`, `DefaultCardsService`, `RulingsService`) downloaded their bulks to a `scryfall-bulk` disk first and read them back via `Storage::disk('scryfall-bulk')->get($fileName)` — which materializes the whole file as a PHP string before `JsonParser` sees it. That worked for `rulings` (25 MB) and `default_cards` (537 MB) given an elevated `memory_limit`, but would have OOM'd on the 2.5 GB `all_cards` bulk; `TranslationsService` therefore pioneered the streaming pattern. The sibling services have since been migrated to the same streaming approach and the `scryfall-bulk` disk has been removed — when copying from any of them as a template, you'll find a uniform `BulkdataService::resolveDownloadUri()` + `JsonParser::parse($uri)` shape.
