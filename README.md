...
Pushing the `v*.*.*` tag triggers the production deploy workflow (see [GitHub Actions workflows](#github-actions-workflows) below). On the next deploy `php artisan config:cache` re-reads `package.json` and the new value is reflected in `config('app.version')`.
...
1. Bump `package.json` `version` (this is the single source of truth — see [Versioning](#versioning) above).
